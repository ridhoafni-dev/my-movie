...
import 'package:response/tv/tv_response.dart';
...
class TvRemoteDataSourceImpl implements TvRemoteDataSource {
  ...
  Future<TvDetailResponse> getTvDetail(int id) async {
    try {
      final response =
          await client.get(Uri.parse('$BASE_URL/tv/$id'), headers: {
  ...
  Future<List<TvModel>> getNowPlayingTvSeries() async {
    try {
      final response =
          await client.get(Uri.parse('$BASE_URL/tv/airing_today'), headers: {
        "Accept": "application/json",
        "Authorization": "Bearer $ACCESS_TOKEN",
      });
      if (response.statusCode == 200) {
  ...
  Future<List<TvModel>> searchTvSeries(String query) async {
    try {
      final response = await client
          .get(Uri.parse('$BASE_URL/search/tv?&query=$query'), headers: {
        "Accept": "application/json",
        "Authorization": "Bearer $ACCESS_TOKEN",
      });
